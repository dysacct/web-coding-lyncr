...
async function sendRegisterCode() {
  ...
  try {
    const data = await sendCodeApi(formData.phone, CAPTCHA_PURPOSE.register)
    if (data.code === 0) {
      showToast('验证码已发送', 'success')
  ...
  <div
      ...
      <template v-if="authMode === 'register'">
        <div class="space-y-5 animate-fade-in animate-duration-200">
          ...
          <div class="flex gap-3">
            <input v-model="formData.registerCode" type="text" inputmode="numeric" maxlength="6" placeholder="请输入验证码"
        ...
        <button type="button"
          ...
          :disabled="submitting" @click="handleRegister">
          {{ submitting ? '注册中...' : '立即注册' }}
        </button>

        <p class="text-center text-sm text-zinc-500 mt-6">
          已有账号？
          <span class="text-blue-400 hover:underline cursor-pointer" @click="switchAuthMode('login')">
            去登录
          </span>
        </p>
      </template>
      ...
      <template v-else>
        ...
        <template v-else>
          ...
          <div v-if="loginType === 'password'" class="space-y-5 animate-fade-in animate-duration-200">
            ...
            <div class="flex items-center justify-between text-[13px] text-zinc-400 px-1 pt-1">
              <div class="flex items-center gap-2.5 cursor-pointer select-none group"
                ...
                <div class="w-[18px] h-[18px] rounded border transition-all flex items-center justify-center shrink-0"
          ...
          <div v-if="loginType === 'qrcode'"
            class="flex flex-col items-center justify-center py-4 animate-fade-in animate-duration-200">
            <div class="relative p-3 bg-white rounded-2xl shadow-2xl mb-5 group cursor-pointer">
              <img src="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=https://github.com/codercat"
                alt="微信扫码登录" class="w-[170px] h-[170px] transition-all duration-300 filter group-hover:blur-xs" />
              <div
                class="absolute inset-0 bg-black/70 rounded-2xl flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            ...
            <div class="mt-7 text-sm text-zinc-400 hover:text-white transition-colors cursor-pointer flex items-center"
              @click="switchMode('password')">
          ...
          <div v-if="loginType !== 'qrcode'"
            class="flex items-start gap-2.5 text-[13px] text-zinc-500 mt-6 px-1 select-none">
            <div
              class="mt-0.5 w-[18px] h-[18px] rounded border transition-all flex items-center justify-center shrink-0 cursor-pointer"
          ...
          <button v-if="loginType !== 'qrcode'" type="button"
            ...
            :disabled="submitting" @click="handleLogin">
            {{ submitting ? '登录中...' : '立即登录' }}
          </button>
          ...
          <p v-if="loginType !== 'qrcode'" class="text-center text-sm text-zinc-500 mt-5">
            没有账号？
            <span class="text-blue-400 hover:underline cursor-pointer" @click="switchAuthMode('register')">
              立即注册
            </span>
          </p>
    ...
    <Transition enter-active-class="transition-all duration-300 ease-out" enter-from-class="opacity-0 -translate-y-4"
      ...
      <div v-if="toastMessage"
        ...
        :class="toastType === 'success' ? 'bg-emerald-600/90' :
